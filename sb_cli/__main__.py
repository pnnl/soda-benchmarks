...
def _add_init_parser(subparsers: argparse._SubParsersAction) -> None:
    p = subparsers.add_parser("init", help="Scaffold a new experiment folder")
    p.add_argument(
        "--output_dir",
        required=True,
    ...
    p.add_argument(
        "--benchmark_name",
        default=None,
        help="Dotted Python import path (e.g. PolyBenchPyTorch...gemm)",
    )
...
def _add_fork_parser(subparsers: argparse._SubParsersAction) -> None:
    p = subparsers.add_parser("fork", help="Fork an existing experiment")
    p.add_argument(
        "--from",
...
def _add_collect_parser(subparsers: argparse._SubParsersAction) -> None:
    p = subparsers.add_parser("collect", help="Collect metrics from experiments")
    p.add_argument(
        "--from",
        dest="from_name_or_path",
        default=None,
...
def main() -> None:
    ...
    if args.command == "init":
        ...
        scaffold(config, args.output_dir, Path.cwd())
        ...
        fork_experiment(args.from_name_or_path, args.output_dir, Path.cwd())
        ...
        collect_command(args.from_name_or_path, Path.cwd())
