...
def main():
    ...
    # Create a model (downloaded mobilenet_v3_small from torchvision)
    model = models.mobilenet_v3_small(weights=models.mobilenet.MobileNet_V3_Small_Weights.DEFAULT)
    model.train(False)
    print(model)

    # Prepare directory and input data: input shape updated for mobilenetv3 (3 channels, 224x224)
    os.makedirs(os.path.dirname(args.out_mlir_path), exist_ok=True)
    in_shape = {'bs': 4, 'c': 3, 'h': 32, 'w': 32}
