...
// RUN: mlir-opt -allow-unregistered-dialect -mlir-elide-elementsattrs-if-larger=2   --load-pass-plugin=%sodap_libs/SODAPlugin%shlibext --pass-pipeline="builtin.module(soda-view-op-graph{print-data-flow-edges=false print-control-flow-edges=true})" %s -o %t 2>&1 | FileCheck -check-prefix=CFG %s
