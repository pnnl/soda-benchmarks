...
// RUN:   --pass-pipeline="builtin.module(soda-instr-change-location)" |\
// RUN:   FileCheck %s

module {
  // CHECK: func.func private @sodaInstrChangeLocation(index)
  ...
  func.func @instr_for(%arg0: memref<8x16xf32>, %arg1: memref<16x8xf32>, %arg2: memref<8x8xf32>) -> memref<8x8xf32> {
    ...
    scf.for %arg3 = %c0 to %c8 step %c1 {
      ...
      // CHECK: func.call @sodaInstrChangeLocation(%[[LOC3]]) : (index) -> ()
      scf.for %arg4 = %c0 to %c8 step %c1 {
        // CHECK: %[[LOC4:.*]] = arith.constant 1 : index
        // CHECK: func.call @sodaInstrChangeLocation(%[[LOC4]]) : (index) -> ()
        scf.for %arg5 = %c0 to %c16 step %c1 {
          // CHECK: %[[LOC5:.*]] = arith.constant 0 : index
          // CHECK: func.call @sodaInstrChangeLocation(%[[LOC5]]) : (index) -> ()
    ...
    // IP as "print every location's final count once".
    // CHECK: %[[REPORT_LOC:.*]] = arith.constant -1 : index
    // CHECK: @sodaInstrChangeLocation(%[[REPORT_LOC]]) : (index) -> ()
